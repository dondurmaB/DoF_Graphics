...
TransformOrder transformOrder = TransformOrder::ScaleRotateTranslate;
...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        float intensity = animateIntensity
            ...
            : staticIntensity;
        float currentRotationDegrees = animateRotation
            ? time * rotationSpeed
            : rotationDegrees;

        // Start with identity: no translation, rotation, or scaling.
        glm::mat4 transform = glm::mat4(1.0f);
        // Matrix multiplication is not commutative; changing order changes the result.
        if (transformOrder == TransformOrder::ScaleRotateTranslate) {
            // GLM post-multiplies here: final matrix T * R * S applies scale, then rotation, then translation.
            transform = glm::translate(transform, glm::vec3(translationX, translationY, 0.0f));
            transform = glm::rotate(transform, glm::radians(currentRotationDegrees), glm::vec3(0.0f, 0.0f, 1.0f));
            transform = glm::scale(transform, glm::vec3(scaleX, scaleY, 1.0f));
        } else {
            // Final matrix S * R * T applies translation, then rotation, then scale.
            transform = glm::scale(transform, glm::vec3(scaleX, scaleY, 1.0f));
            transform = glm::rotate(transform, glm::radians(currentRotationDegrees), glm::vec3(0.0f, 0.0f, 1.0f));
            transform = glm::translate(transform, glm::vec3(translationX, translationY, 0.0f));
        }
