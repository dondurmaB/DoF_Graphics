...
// ==============================
// EXPERIMENT 06 CONTROLS
// ==============================
bool enableDepthTest = true;
GLenum depthFunction = GL_LESS;

bool drawSecondCube = true;
bool reverseCubeDrawOrder = false;

float cubeAOffsetX = -0.18f;
float cubeAOffsetY = 0.0f;
float cubeAOffsetZ = -0.20f;
float cubeARotationXDegrees = 25.0f;
float cubeARotationYDegrees = 35.0f;
float cubeAUniformScale = 0.55f;

float cubeBOffsetX = 0.18f;
float cubeBOffsetY = 0.0f;
float cubeBOffsetZ = 0.15f;
float cubeBRotationXDegrees = -20.0f;
float cubeBRotationYDegrees = -35.0f;
float cubeBUniformScale = 0.55f;
...
int main() {
    ...
    // Depth testing operates per fragment. With GL_LESS, an incoming fragment passes if its depth is smaller than the stored value.
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        auto drawCube = [&](float offsetX, float offsetY, float offsetZ,
                            float rotationX, float rotationY, float scale) {
            // Code builds T * Ry * Rx * S; with M * vertex, the cube experiences scale -> X rotation -> Y rotation -> translation.
            ...
            // The depth buffer stores a depth value per framebuffer location, so nearer fragments can replace farther ones.
            glDrawElements(GL_TRIANGLES, indexCount, GL_UNSIGNED_INT, nullptr);
        };

        if (reverseCubeDrawOrder) {
            if (drawSecondCube) {
                drawCube(cubeBOffsetX, cubeBOffsetY, cubeBOffsetZ,
                         cubeBRotationXDegrees, cubeBRotationYDegrees, cubeBUniformScale);
            }
            drawCube(cubeAOffsetX, cubeAOffsetY, cubeAOffsetZ,
                     cubeARotationXDegrees, cubeARotationYDegrees, cubeAUniformScale);
        } else {
            drawCube(cubeAOffsetX, cubeAOffsetY, cubeAOffsetZ,
                     cubeARotationXDegrees, cubeARotationYDegrees, cubeAUniformScale);
            if (drawSecondCube) {
        ...
        // With depth testing enabled, correct visibility should depend on depth, not cube submission order.
