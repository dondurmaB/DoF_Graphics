...
// ==============================
// EXPERIMENT 01 CONTROLS
// ==============================
float triangleScale = 1.0f;

float triangleOffsetX = 0.0f;
float triangleOffsetY = 0.0f;

float redVertexColor[3] = {1.0f, 0.0f, 0.0f};
float greenVertexColor[3] = {0.0f, 1.0f, 0.0f};
float blueVertexColor[3] = {0.0f, 0.0f, 1.0f};

// Change this to GL_LINE_LOOP or GL_POINTS, then rebuild, to compare primitives.
...
int main() {
    ...
    float vertices[] = {
        (-0.5f * triangleScale) + triangleOffsetX, (-0.5f * triangleScale) + triangleOffsetY, 0.0f,
        redVertexColor[0], redVertexColor[1], redVertexColor[2],

        (0.5f * triangleScale) + triangleOffsetX, (-0.5f * triangleScale) + triangleOffsetY, 0.0f,
        greenVertexColor[0], greenVertexColor[1], greenVertexColor[2],

        (0.0f * triangleScale) + triangleOffsetX, (0.5f * triangleScale) + triangleOffsetY, 0.0f,
        blueVertexColor[0], blueVertexColor[1], blueVertexColor[2]
    };

    //create Vertex Buffer Object and Buffer
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        glUseProgram(shaderProgram);
        glBindVertexArray(VAO);
        glDrawArrays(primitiveMode, 0, 3);
