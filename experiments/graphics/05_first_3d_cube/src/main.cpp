...
// ==============================
// EXPERIMENT 05 CONTROLS
// ==============================
float translationX = 0.0f;
float translationY = 0.0f;

float uniformScale = 0.7f;

float rotationXDegrees = 25.0f;
float rotationYDegrees = 35.0f;
float rotationZDegrees = 0.0f;

bool animateRotation = false;
float rotationSpeedX = 20.0f;
float rotationSpeedY = 35.0f;
...
int main() {
    ...
    if (transformLocation == -1) {
        cout << "Warning: could not find uniform uTransform" << endl;
    }
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        float intensity = animateIntensity
            ...
            : staticIntensity;
        float currentRotationXDegrees = animateRotation
            ? rotationXDegrees + time * rotationSpeedX
            : rotationXDegrees;
        float currentRotationYDegrees = animateRotation
            ? rotationYDegrees + time * rotationSpeedY
            : rotationYDegrees;
        float currentRotationZDegrees = animateRotation
            ? rotationZDegrees + time * rotationSpeedZ
            : rotationZDegrees;

        // Start with identity: no translation, rotation, or scaling.
        glm::mat4 transform = glm::mat4(1.0f);
        // Code builds T * Rz * Ry * Rx * S; with M * vertex, the cube experiences scale -> X -> Y -> Z -> translate.
        transform = glm::translate(transform, glm::vec3(translationX, translationY, 0.0f));
        transform = glm::rotate(transform, glm::radians(currentRotationZDegrees), glm::vec3(0.0f, 0.0f, 1.0f));
        transform = glm::rotate(transform, glm::radians(currentRotationYDegrees), glm::vec3(0.0f, 1.0f, 0.0f));
        transform = glm::rotate(transform, glm::radians(currentRotationXDegrees), glm::vec3(1.0f, 0.0f, 0.0f));
        transform = glm::scale(transform, glm::vec3(uniformScale, uniformScale, uniformScale));
        ...
        if (transformLocation != -1) {
            // Upload the composed 4x4 matrix as a uniform. The vertex shader applies it to every vertex.
            glUniformMatrix4fv(transformLocation, 1, GL_FALSE, glm::value_ptr(transform));
        }
        glBindVertexArray(VAO);
        // Depth testing is intentionally disabled in Experiment 05 and will be introduced in the next experiment.
