...
// ==============================
// EXPERIMENT 03 CONTROLS
// ==============================
bool animateIntensity = true;
bool animateHorizontalPosition = true;

float intensitySpeed = 1.0f;
float movementSpeed = 1.0f;
float movementAmplitude = 0.35f;

float staticIntensity = 1.0f;
float staticOffsetX = 0.0f;
...
int main() {
    ...
    int offsetLocation = glGetUniformLocation(shaderProgram, "uOffsetX");
    if (offsetLocation == -1) {
        cout << "Warning: could not find uniform uOffsetX" << endl;
    }
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        float intensity = animateIntensity
            ...
            : staticIntensity;
        float offsetX = animateHorizontalPosition
            ? sin(time * movementSpeed) * movementAmplitude
            : staticOffsetX;
