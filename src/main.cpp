...
VisualizationMode visualizationMode = VisualizationMode::Color;
...
int main() {
    ...
    int visualizationModeLocation = glGetUniformLocation(shaderProgram, "uVisualizationMode");
    if (visualizationModeLocation == -1) {
        cout << "Warning: could not find uniform uVisualizationMode" << endl;
    }

    int nearPlaneLocation = glGetUniformLocation(shaderProgram, "uNearPlane");
    if (nearPlaneLocation == -1) {
        cout << "Warning: could not find uniform uNearPlane" << endl;
    }

    int farPlaneLocation = glGetUniformLocation(shaderProgram, "uFarPlane");
    if (farPlaneLocation == -1) {
        cout << "Warning: could not find uniform uFarPlane" << endl;
    }

    int depthVisualizationMaxLocation = glGetUniformLocation(shaderProgram, "uDepthVisualizationMax");
    if (depthVisualizationMaxLocation == -1) {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        auto renderScene = [&]() {
            ...
            if (visualizationModeLocation != -1) {
                glUniform1f(visualizationModeLocation, static_cast<float>(static_cast<int>(visualizationMode)));
            }
            // Reconstruction uses the same near/far values as glm::perspective(); mismatches produce wrong depth.
            if (nearPlaneLocation != -1) {
            ...
            if (depthVisualizationMaxLocation != -1) {
                glUniform1f(depthVisualizationMaxLocation, depthVisualizationMax);
            }
            glBindVertexArray(VAO);
        ...
        if (renderThroughFramebuffer) {
            ...
            // Pass 2: return to the window and copy the finished scene color through a screen quad.
